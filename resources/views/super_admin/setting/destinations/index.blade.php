...
@section('content')
    <div class="p-30">
        <input type="hidden" id="destination-route" value="{{ route('super_admin.setting.destinations.index') }}">
                    ...
                    @foreach ($regionCards as $region)
                        <div class="region_card">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item" href="javascript:void(0)"
                                            onclick="getEditModal('{{ route('super_admin.setting.destinations.regions.edit', $region->id) }}', '#edit-modal')">
                                            <span>{{ __('Edit') }}</span>
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="javascript:void(0)"
                                            onclick="deleteItem('{{ route('super_admin.setting.destinations.regions.delete', $region->id) }}', 'countryDataTable')">
                                            <span>{{ __('Delete') }}</span>
                                        </a>
                                    </li>
                                </ul>
                            ...
                            <div class="cards-body">
                                ...
                                <div class="subtitle">
                                    <span class="globe-icon"><img src="{{ asset('assets/images/icons/earth.svg') }}"
                                            width="10" height="10" alt="icon"></span> {{ $region->code }}
                                </div>
                            </div>
                            <div class="cards-footer">
                                ...
                                <div class="dashboared-status-badge {{ $region->status == STATUS_ACTIVE ? 'active' : 'pending' }}">
            ...
            <div class="tab-pane fade" id="countries-pane" role="tabpanel" aria-labelledby="countries-tab">
                <div class="dashboard_search_addNew_box">
                    ...
                    <button type="button" class="common_button add_new_button" data-bs-toggle="modal"
                        data-bs-target="#addCountryModal">+ {{ __('Add Country') }}</button>
                </div>
                ...
                <form class="ajax reset" action="{{ route('super_admin.setting.destinations.countries.store') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            ...
                            <div class="dashboard-form-group">
                                <label>{{ __('Country Code List') }}</label>
                                <select name="continent" class="select2-activate">
                        ...
                        <div class="form-actions">
                            <button type="button" class="common_button btn-cancel"
                                data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                            <button type="submit" class="common_button add_new_button">{{ __('Add Country') }}</button>
                        </div>
                    </div>
                </form>
            </div>
